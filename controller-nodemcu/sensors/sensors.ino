...
//String supportedsensors[] = {"DHT","PHOTOR","USONIC"};// supported sensor list

String sensors[] = {"DHT","PHOTOR","LED"};// select connected senssors from supportedsensors and write here
String sensorsstatus[] = {"ON","OFF","OFF"};// select connected senssors from supportedsensors and write here
   ...
   for( int i=0; i<10; i++){
      ...
      if ( getStatus(sensors[i]) == "ON" ) {
        if ( actdiffTime >= postdelaytime ){ // delaytime manage
          ...
          if ( sensors[i] == "LED") { loop_led(); loop_http(); }
